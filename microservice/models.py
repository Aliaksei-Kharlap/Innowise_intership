...
class UserPage(Base):
    __tablename__ = Table("facebookk_page_followers", metadata, autoload=True, autoload_with=engine)
...
class Page(Base):
    __tablename__ = Table("facebookk_page", metadata, autoload=True, autoload_with=engine)
    id = Column(Integer, primary_key=True)
